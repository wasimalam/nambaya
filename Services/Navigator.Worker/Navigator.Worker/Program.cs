...
using Navigator.Worker.Consumer;
using Serilog;
...
namespace Navigator.Worker
{
    internal class Program
    {
            ...
            try
            {
                ...
                var builder = new HostBuilder()
                    ...
                    .ConfigureServices((hostContext, services) =>
                    {
                        ...
                        if (args.Contains("--import") == false)
                        {
                            services.AddMassTransit(cfg =>
                            {
                                cfg.AddConsumer<PatientConsumer>();
                                //cfg.AddConsumer<CardiologistInsertConsumer>();
                                //cfg.AddConsumer<CardiologistUpdateConsumer>();
                        ...
                        else
                        {
                            ...
                            .WithParsed<ImportOptions>(o =>
                            {
                                if (o.import)
                                {
                                    Console.WriteLine($"Import process enabled.");
                                    Console.WriteLine($"Going to import pharmacy file at {o.PharmacyFile}");
        ...
        private static IBusControl ConfigureBus(IServiceProvider provider)
        {
            ...
            return Bus.Factory.CreateUsingRabbitMq(cfg =>
            {
                ...
                //cfg.ReceiveEndpoint(KnownChannels.NAVIGATOR_CREATE_CARDIOLOGIST, ep =>
                //{
                //    ep.PrefetchCount = 16;
                //    ep.UseMessageRetry(mr => mr.Interval(2, 100));
                //    ep.ConfigureConsumer<CardiologistInsertConsumer>(provider);
                //});
                //cfg.ReceiveEndpoint(KnownChannels.NAVIGATOR_UPDATE_CARDIOLOGIST, ep =>
                //{
                //    ep.PrefetchCount = 16;
                //    ep.UseMessageRetry(mr => mr.Interval(2, 100));
                //    ep.ConfigureConsumer<CardiologistUpdateConsumer>(provider);
                //});
    ...
    {
        [Option('i', "import", Required = false, HelpText = "Start import process")]
        public bool import { get; set; }
        [Option('p', "pharmacyfile", Required = true, HelpText = "Set pharmacy data csv file path")]
